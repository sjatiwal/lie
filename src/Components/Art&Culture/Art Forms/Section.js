import React from "react";
import { useState } from "react";
import { Link } from "react-router-dom";

import Dance from "./content/Dance";
import MartialArt from "./content/MartialArt";
import Music from "./content/Music";
import Paintings from "./content/Paintings";
import Pottery from "./content/Pottery";
import Puppetry from "./content/Puppetry";
import Sculpture from "./content/Sculpture";
import Theatre from "./content/Theatre";

export const ROUTES = [
  {
    path: "dance",
    element: <Dance />,
    name: "Dance",
  },
  {
    path: "martial-art",
    element: <MartialArt />,
    name: "Martial Art",
  },
  {
    path: "music",
    element: <Music />,
    name: "Music",
  },
  {
    path: "painting",
    element: <Paintings />,
    name: "Painting",
  },
  {
    path: "pottery",
    element: <Pottery />,
    name: "Pottery",
  },
  {
    path: "puppetry",
    element: <Puppetry />,
    name: "Puppetry",
  },
  {
    path: "sculpture",
    element: <Sculpture />,
    name: "Sculpture",
  },
  {
    path: "theatre",
    element: <Theatre />,
    name: "Theatre",
  },
];
function Section() {
  const [display, setDisplay] = useState(false);

  return (
    <div>
      {" "}
      <div
        onClick={() => setDisplay(!display)}
        className="  text-[.7rem] bg-slate-400 pl-[.8em] pt-[.3em] pb-[.2em] pr-[.3em]"
      >
        Menu
      </div>
      <div>
        {display ? (
          <div className="section h-screen overflow-scroll bg-gray-300  bottom-0   pt-[.2em] pl-[.2em] pr-[.2em] pb-[0]">
            {ROUTES.map((item) => {
              return (
                <div className="hover:bg-slate-400" key={`${item.name}`}>
                  <Link to={`/art-culture/art-forms/${item.path}`}>
                    {item.name}
                  </Link>
                </div>
              );
            })}
          </div>
        ) : null}
      </div>
    </div>
  );
}

export default Section;
